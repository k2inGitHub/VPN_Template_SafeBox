...
@interface PhotoMoveController ()

@end

@implementation PhotoMoveController
...
- (void)viewDidLoad
{
    ...
    self.navigationItem.title = NSLocalizedString(@"Move photos", @"Move photos");
    ...
    UIBarButtonItem *right = [[UIBarButtonItem alloc] initWithTitle:NSLocalizedString(@"Done", @"Done") style:UIBarButtonItemStylePlain target:self action:@selector(bbiAction:)];
    right.tag = 10;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.detailTextLabel.text = [NSString stringWithFormat:@"%d %@", item.files.count, NSLocalizedString(@"photos", @"photos")];
